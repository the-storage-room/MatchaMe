...
const FollowsItem = ({
  ...
  const statusFinder = () => {
    if (!activevoting && firstAccept === undefined) {
      ...
      const rejecter =
        user1.id === firstRejection ? user1.firstname : user2.firstname;
      ...
      const rejecter2 =
        user1.id === firstRejection ? user1.firstname : user2.firstname;
      const accepter =
        user1.id === firstAccept ? user1.firstname : user2.firstname;
    ...
    <div className={style.followItem}>
      <div className={style.user1}>
        <img
          ...
          src={user1.photos[0].url || user1.photos[0]}
        />
      ...
      <div className={style.user2}>
        <img
          className={style.avatar}
          src={user2.photos[0].url || user2.photos[0]}
        />
        <h3 className={style.h3}>
      ...
      <div onClick={() => onClick(matchId, starred, index)}>
        <img
          className={starred ? style.starLeaf : style.unstarLeaf}
          src="http://www.learnandflourish.com/wp-content/uploads/2013/01/small-leaf2.png"
        />
      </div>
    </div>
  );
